...
function _GetDB
{
	...
	if($Database.ExchangeVersion.ExchangeBuild.Major -ge 14 -and $E2010)
	{
        ...
        # 2015-02-26 Thomas Stensitzki, try/catch added
        try {
		    $ArchiveStatistics = [array]($ArchiveMailboxes | Where {$_.ArchiveDatabase -eq $Database.Name} | Get-MailboxStatistics -Archive -ErrorAction SilentlyContinue )
        }
        catch {
            Write-Warning "Cannot get archive mailbox statistics for $($Database.Name)"
        }
        ...
        # 2015-02-26 Thomas Stensitzki, try/catch added
        try {
		    $Size = [long](Get-WmiObject cim_datafile -ComputerName $Database.Server.Name -Filter ('name=''' + $Database.edbfilepath.pathname.replace("\","\\") + '''') -ErrorAction SilentlyContinue).filesize 
        }
        catch { }
		if (!$Size)
		{
...
function _GetExSvr
{
	...
	if ($ExchangeMajorVersion -ge 8)
	{
		...
		if ($Roles -contains "Mailbox")
		{
            ...
            # 2015-02-26 Thomas Stensitzki, try/catch added
            try {
			    $MailboxStatistics = Get-MailboxStatistics -Server $ExchangeServer -ErrorAction SilentlyContinue | Select DisplayName,@{Name="TotalItemSizeB";Expression={$_.TotalItemSize.Value.ToBytes()}},@{Name="TotalDeletedItemSizeB";Expression={$_.TotalDeletedItemSize.Value.ToBytes()}},Database
            }
            catch {
                $MailboxStatistics = $null
                Write-Warning "Cannot get mailbox statistics for server $ExchangeServer"
            }
	    }
        ...
        # 2015-05-26 Thomas Stensitzki, try/catch added
        try {
...
{
    # 2015-02-26 Thomas Stensitzki, Support for Exchange Scripts located in non-default locations
    # Use $env:ExchangeInstallPath for Exchange 2010/2013 Setups
    ...
    Write-Verbose "Exchange Install Path: $ExchangeInstallPath"
	...
	} elseif (Test-Path $LocalExchangePath) {
...
if ((Get-PSSnapin -Name Microsoft.Exchange.Management.PowerShell.Admin -ErrorAction SilentlyContinue))
{
    ...
    #$localserver = (Get-ItemProperty -Path HKLM:\SOFTWARE\Microsoft\ExchangeServer\v15\Setup).MsiProductMajor
    # 2015-02-26 Thomas Stensitzki, Support for Exchange 2013 Management Only Servers
...
$ExMajorVersionStrings = @{"6.0" = @{Long="Exchange 2000";Short="E2000"}
                           ...
                           "16"  = @{Long="Exchange 2016";Short="E2016"}}
